...
class Compose(object):
    ...
    def __call__(self, image, rImg=None, label=None, bbox=None):
        for t in self.segtransform:
            image, rImg, label, bbox = t(image, rImg, label, bbox)
        return image, rImg, label, bbox
    ...
    def __call__(self, image, rImg=None, label=None, bbox=None):
        ...
        if label is not None:
            ...
            if not isinstance(label, torch.LongTensor):
                label = label.long()
        if rImg is not None:
            rImg = torch.from_numpy(rImg.transpose((2, 0, 1)))
            if not isinstance(rImg, torch.FloatTensor):
                rImg = rImg.float()
    ...
    def __call__(self, image, rImg=None, label=None, bbox=None):
        if self.std is None:
            for t, m in zip(image, self.mean):
                t.sub_(m)
            if rImg is not None:
                for t, m in zip(rImg, self.mean):
                    t.sub_(m)
        else:
            for t, m, s in zip(image, self.mean, self.std):
                t.sub_(m).div_(s)
            if rImg is not None:
                for t, m, s in zip(rImg, self.mean, self.std):
                    t.sub_(m).div_(s)
...
class RandScale(object):
    ...
    def __call__(self, image, rImg=None, label=None, bbox=None):
        ...
        #case right Image
        if rImg is not None:
            rImg = cv2.resize(rImg, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_LINEAR)

        #case segmentation label
        if label is not None:
            label = cv2.resize(label, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)

        #case detection label
        if bbox is not None:
            bbox = bbox.copy()
            if "boxes" in bbox:
                boxes = bbox["boxes"]
...
class Crop(object):
    ...
    def __call__(self, image, rImg=None, label=None, bbox=None):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            if rImg is not None:
                rImg = cv2.copyMakeBorder(rImg, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.padding)
            if label is not None:
        ...
        if rImg is not None:
            rImg = rImg[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
...
class RandomColorJitter(object):
    ...
    def __call__(self, image, rImg=None, label=None, bbox=None):
        if random.random() < self.p:
            ...
            if rImg is not None:
                rImg = self.to_pil(rImg)
                rImg = self.color_aug(rImg)
