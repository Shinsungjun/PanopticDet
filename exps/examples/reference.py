...
class Exp(BaseExp):
    ...
    def get_model(self):
        from engine.models import Fullmodel, Ctxnet, Head
        ...
        if getattr(self, "model", None) is None:
            ...
            backbone = Ctxnet()
            head = Head()
            self.model = Fullmodel(backbone, head)
        ...
        with wait_for_the_master(local_rank):
            dataset = Cityscapes(
                ...
                transforms=transform.Compose([ #% init에 parameter 추가
                    ...
                    transform.Crop([768,1536], crop_type='rand', padding=self.mean, ignore_label=255),
                    transform.ToTensor(),
    ...
    def get_lr_scheduler(self, lr, iters_per_epoch):
        from engine.utils import LRScheduler
    ...
    def get_eval_loader(self, batch_size, is_distributed, testdev=False, legacy=False):
        from engine.data import COCODataset, ValTransform
